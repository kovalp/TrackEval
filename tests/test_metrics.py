...
def no_confusion():
    ...
    expected = {
        'clear': {'CLR_TP': 4, 'CLR_FN': 4, 'CLR_FP': 5, 'IDSW': 0, 'MOTA': 1 - 9 / 8},
        'identity': {
            'IDTP': 4,
            'IDFN': 4,
            'IDFP': 5,
        ...
        'vace': {'STDA': 2 / 5 + 2 / 4, 'ATA': (2 / 5 + 2 / 4) / 2},
    }
...
def split_tracks():
    ...
    expected = {
        ...
        'vace': {'STDA': 2 / 4 + 2 / 5, 'ATA': (2 / 4 + 2 / 5) / (0.5 * (2 + 5))},
    }
...
def _from_dense(
    ...
    similarity_subset = [
        similarity[t][gt_subset[t], :][:, tracker_subset[t]] for t in range(num_timesteps)
    ]
    data = {
        'num_timesteps': num_timesteps,
        'num_gt_ids': num_gt_ids,
