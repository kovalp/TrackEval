    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                ...
                if is_gt:
                    gt_extras_dict = {'truncation': np.empty(0), 'occlusion': np.empty(0)}
                    raw_data['gt_extras'][t] = gt_extras_dict
                else:
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            gt_class_mask = np.sum(
                [raw_data['gt_classes'][t] == c for c in [cls_id] + distractor_classes], axis=0
            )
            ...
            intersection_with_ignore_region = self._calculate_box_ious(
                unmatched_tracker_dets, crowd_ignore_regions, box_format='x0y0x1y1', do_ioa=True
            )
