...
class JAndF(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        if frame_shape:
            ...
            for t in range(num_timesteps):
                ...
                tracker_id_det_mapping = {
                    tracker_ids[t][i]: tracker_dets[t][i] for i in range(len(tracker_ids[t]))
                }
                tracker_dets[t] = [
        ...
        res = {
            ...
            'J-Recall': [
                np.nanmean(j_m[i, :] > 0.5 + np.finfo('float').eps) for i in range(j_m.shape[0])
            ],
            'F-Mean': [np.nanmean(f_m[i, :]) for i in range(f_m.shape[0])],
            'F-Recall': [
                np.nanmean(f_m[i, :] > 0.5 + np.finfo('float').eps) for i in range(f_m.shape[0])
            ],
            'J-Decay': [],
            'F-Decay': [],
        }
